...
def statis_species(filepath,outpath):
    ...
    df_all['null_count']=(df_all == 0).astype(int).sum(axis=1)
    print(df_all.head())
    print("df_all length")
    print(len(df_all))
    df_all1=df_all[df_all['null_count'] <= all_sample_count*0.75]
    df_all1=df_all1[['mean','null_count']]
    df_all1=df_all1.sort_values(by = ['mean'],ascending = False)
    print(df_all1.head())
    print(len(df_all1))
    df_all1.to_csv(outpath,sep='\t',header=False)
    print("finished")
def main():
    filepath="/home/zc/IDBdata/all_data/all-taxonomy-IBD"
    outpath="/home/zc/IDBdata/all_data/species-analysis/ibd-species-common0.25-ave.csv"
